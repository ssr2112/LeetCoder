...
private:
    void findPre(Node *root, int key, Node *&pre){
        if(!root) return;
        if(root->key>=key) findPre(root->left, key, pre);
        else{
           if(!pre || abs(key-root->key) < abs(key-pre->key)) pre = root;
    ...
    void findSucc(Node *root, int key, Node *&suc){
        if(!root) return;
        if(root->key<=key) findSucc(root->right, key, suc);
        else{
           if(!suc || abs(key-root->key) < abs(key-suc->key)) suc = root;
